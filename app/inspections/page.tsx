      ...
      <main className="container">
        <div className="stack">
          {inspections && inspections.length > 0 ? (
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", marginBottom: 8 }}>
                  <div style={{ fontWeight: 800 }}>
                    {insp.scheduled_at
                      ? new Date(insp.scheduled_at).toLocaleDateString("de-DE", {
                          day: "2-digit",
                          month: "2-digit",
                          year: "numeric",
                          hour: "2-digit",
                ...
                {insp.assigned_user ? (
                  <div className="muted" style={{ marginTop: 6, fontSize: 12 }}>
                    Assigned to: <strong>{insp.assigned_user.full_name || insp.assigned_user.email || "Unknown"}</strong>
                  </div>
                ) : insp.assigned_to ? (
                  <div className="muted" style={{ marginTop: 6, fontSize: 11, wordBreak: "break-all" }}>
                    Assigned to: {insp.assigned_to.slice(0, 8)}…
                  </div>
                ) : null}
                <div className="muted" style={{ marginTop: 6, fontSize: 11, wordBreak: "break-all" }}>
                  {insp.id.slice(0, 8)}…
            ...
            <div className="card">
              <div style={{ fontWeight: 800 }}>No inspections yet</div>
              <div className="muted" style={{ marginTop: 6 }}>
                {canCreate
                  ? "Create a lead first, then create an inspection from it."
                  : "No inspections have been assigned to you yet."}
              </div>
              {canCreate ? (
                <div style={{ marginTop: 12 }}>
                  <Link className="btn" href="/inspection/new">
                    Create inspection
                  </Link>
                </div>
              ) : null}
